...
while true; do
  ...
  while [ -z "$pid" ]; do
    ...
    /usr/sbin/rpc.nfsd --debug 8 --no-udp --no-nfs-version 3 --nfs-version 4.1 --nfs-version 4.2
    echo "Exporting File System..."
    # Export the filesystem without verification to avoid hanging
    /usr/sbin/exportfs -ra
    echo "Export completed"
    echo "Starting Mountd in the background..."
    /usr/sbin/rpc.mountd --debug all --no-udp --no-nfs-version 3 --nfs-version 4.1 --nfs-version 4.2
